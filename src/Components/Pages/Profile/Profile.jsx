...
const Profile = () => {
    ...
    const handleEnter = (event) => {
        if (event.key === 'Enter' && skill.trim().length > 0) {
            dispatch(addSkill(skill, user.id, toast));
            setSkill('');
        }
    };
    ...
    const handleDeleteSkill = (skill) => {
        dispatch(deleteSkill(skill, toast));
    }

    const handleChangeName = (e) => {
        e.preventDefault();
        if(userName.trim().length > 0){
            dispatch(changeUserName(userName, user.id, toast));
            setUserName('');
        }
    }
            ...
            <div className='profile__block'>
                <div className='profile__block-left'>
                    <IconButton sx={{ display:'flex', flexDirection:'column', position:'relative' }} component="label" htmlFor="photo">
                        ...
                        <Input type="file" name="photo" id="photo" onChange={handleFileChange} sx={{ display: 'none' }} disableUnderline />
                                ...
                                <Stack direction="row" sx={{ display:'flex', alignItems:'center', flexWrap:'wrap'}}>
                                    {skills?.map(skill => 
                                        <Box sx={{backgroundColor:'rgb(155,155,155,0.5)', margin:'2px', padding:'8px 12px', borderRadius:'20px', display:'flex', alignItems:'center'}}>
                                            ...
                                            <CloseIcon onClick={() => handleDeleteSkill(skill)} sx={{ width:16, height:16, fill:'gray', ml: 0.6, cursor:'pointer' }} />
                                        </Box>
                                    )}
                                </Stack>
                            </Box>
